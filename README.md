...
第一次使用的引導會沿用上述 `A+B` 範例；除非你已提出真實需求或要求其他範例，AI 不會臨時換成待辦清單等隨機功能。提出需求後先確認本需求的模式；若可觀察行為、範圍、驗收條件、重要邊界／錯誤、風險取捨或測試方式仍有未決事項，AI 會自動使用 `grilling` 逐輪協助釐清，不需要另外指定 Skill。需求已完整且可測時，則直接整理 Spec，避免不必要的訪談。

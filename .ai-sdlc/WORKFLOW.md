...
1. 先讀取現有程式、專案規則與相鄰功能。
2. 只詢問會改變可觀察行為、範圍、錯誤處理、風險或測試方式的問題。
3. 使用專案既有 Spec 位置；沒有慣例時，從 [Spec 範本](templates/SPEC.md) 建立 `docs/specs/<feature>.md`。
4. Spec 至少寫清楚目標、可驗收條件、重要邊界／錯誤、非目標與驗證方式。
5. 先做 Spec Review，確認內容一致且可測試，再請使用者確認。
6. 使用者確認後才開始實作。Spec 與程式留在同一個 Feature Branch，不另外建立 Spec Branch 或 Spec PR。
7. 若實作期間必須改變產品行為，更新同一份 Spec、說明影響並重新取得確認，再繼續。

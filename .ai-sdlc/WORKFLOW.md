...
- 每個新需求都重新選擇監督、委派或全自動模式，不沿用上一個需求。
...
如果初次提示沒有產品需求，完成後停止並等待使用者提出需求；即使 README 提供 `A+B` 範例，也不得自動開始。如果初次提示已包含真實需求，先保存需求內容，完成上述設定交付後再進入下列「開始新需求」與模式選擇。

初始設定完成且尚無產品需求時，下一步引導必須沿用 README 的第一次使用說明：提示使用者可以用 `A+B` 熟悉流程，也可以直接提出真實需求，並提供「需求／模式」輸入格式。除非使用者已提出其他需求或要求其他範例，不得臨時以待辦清單或其他隨機功能取代 `A+B`。這段文字只提供選項，不代表自動開始範例需求。

## 2. 開始新需求

### 2.1 固定起手步驟：選擇模式

每個新需求都是新的執行流程。若使用者尚未指定模式，在建立 Feature Branch 或修改產品檔案前詢問：
...
| 模式 | AI 可自動執行 | 固定停止點 |
| --- | --- | --- |
| 監督模式 | 依使用者逐階段授權執行 | Spec 確認、實作與本機驗證完成、每次 Push／更新 PR，以及 Merge 前等待確認 |
| 委派模式 | Spec 確認後，自動實作、驗證、修正、Commit、Push、建立與更新 PR | Merge 前等待使用者 |
| 全自動模式 | Spec 確認後，在已授權範圍內自動執行同一流程 | 沒有一般停止點；全部檢查通過後自動 Merge |

三種模式都必須執行相同的 Spec、Test、Review 與 CI。需求歧義、需要改變 Spec、系統安裝、新權限或無法安全判斷時，任何模式都必須詢問使用者。

使用者指定 `模式：全自動` 或說「切換為全自動模式」，即代表目前需求已授權在所有條件成立後自動 Merge，不需要再取得一次 Merge 確認；Repository 權限與保護規則仍須遵守。
...
切換從下一個尚未執行的動作生效，不建立新分支、不重做需求，也不取消已完成的有效工作。AI 應回覆目前模式、Feature Branch、下一步與下一個停止點。

### 2.3 建立 Feature Branch
...
5. 使用專案既有 Spec 位置；沒有慣例時，從 [Spec 範本](templates/SPEC.md) 建立 `docs/specs/<feature>.md`。
6. Spec 至少寫清楚目標、可驗收條件、重要邊界／錯誤、非目標與驗證方式。
7. 先做 Spec Review，確認內容一致且可測試，再請使用者確認。
8. 使用者確認後才開始實作。Spec 與程式留在同一個 Feature Branch，不另外建立 Spec Branch 或 Spec PR。
9. 若實作期間必須改變產品行為，更新同一份 Spec、說明影響並重新取得確認，再繼續。
...
- 符合「3.1 釐清並確認 Spec」的未決條件時必須自動使用 `grilling`；需求已完整且可測時不強制啟用。
